...
const propiedadesFiltradas = propiedades.filter(p => {
  // Excluye las propiedades que tienen el estado "ARCHIVAR"
  if (p.estado === "Archivada") return false;

  // Filtro específico para solo mostrar propiedades de alquiler
  ...
  const coincideExtranjero = !filtros.soloExtranjeros || p.esExtranjero;
  ...
  const coincideOferta = filtros.oferta === "Todos" || p.operacion === filtros.oferta;
      ...
      <header className="sticky top-0 z-50 w-full bg-black/50 backdrop-blur-md border-b border-white/10 px-10 py-6 flex items-center justify-between text-white">
        ...
        {usuario ? (
          <div className="flex items-center gap-6 bg-white/5 px-6 py-3 rounded-full border border-white/10 backdrop-blur-md">
            <span className="text-2xl cursor-pointer transition-transform duration-300 hover:-translate-y-1">
  🔔
  </span>
            <Link href="/perfil" className="transition-transform duration-300 hover:-translate-y-1">
              <img 
                src={datosPerfil?.fotoUrl?.trim() ? datosPerfil.fotoUrl : "https://ui-avatars.com/api/?name=User"} 
                className="w-10 h-10 rounded-full border border-white/30 object-cover" 
                alt="Perfil" 
              />
            </Link>
            <button onClick={handleLogout} className="text-red-500 font-black text-sm uppercase hover:text-white transition-colors duration-300">SALIR</button>
          </div>
        ) : (
      ...
      <section className="py-16 flex flex-col items-center text-center text-white">
        <h1 className="text-5xl font-black mb-4">Encuentra Tu Espacio Ideal</h1>
        <p className="text-gray-400 mb-8 max-w-md">
          Explora nuestra selección de <span className="text-red-500 font-black">{propiedadesFiltradas.length}</span> propiedades premium, diseñadas para elevar tu estilo de vida.
        </p>
        
        <div className="flex gap-4">
          <button 
            onClick={() => setSoloExclusivas(!soloExclusivas)}
            className={`${soloExclusivas ? "bg-amber-600" : "bg-white/10"} hover:bg-amber-700 text-white px-8 py-3 rounded-full font-bold shadow-lg shadow-amber-900/20 transition-all`}>
            ⭐ {soloExclusivas ? "MOSTRANDO LO MÁS EXCLUSIVO" : "LO MAS EXCLUSIVO"}
          </button>
          <button 
            onClick={() => setMostrandoFavoritos(!mostrandoFavoritos)}
            className={`${mostrandoFavoritos ? "bg-red-600" : "bg-white/10"} hover:bg-red-700 text-white px-8 py-3 rounded-full font-bold shadow-lg shadow-red-900/20 transition-all`}>
            ❤️ {mostrandoFavoritos ? "Ver Todas" : "Ver Mis Favoritos"}
          </button>
        </div>
      </section>
      ...
      <div className="flex p-6 gap-8">
        <aside className="w-[340px] bg-[#111] border border-white/10 rounded-3xl p-6 h-fit sticky top-24 backdrop-blur-xl">
          <h2 className="text-lg font-bold mb-6 flex items-center gap-2 text-white">
            <span className="w-1 h-6 bg-red-500 rounded-full"></span> Filtros de Búsqueda
          </h2>
          <div className="space-y-5">
            <div>
              <label className="text-[10px] font-bold text-gray-400 uppercase mb-2 block">Búsqueda</label>
            ...
            <Selector label="Oferta" opciones={["Todos", "Alquiler", "Venta"]} value={filtros.oferta} onChange={(v) => setFiltros({...filtros, oferta: v})} />
            <Selector label="Habs" opciones={["Cualquiera", "1", "2", "3", "4+"]} value={filtros.habs} onChange={(v) => setFiltros({...filtros, habs: v})} />
            <div className="flex items-center gap-3 pt-4 border-t border-white/10">
              <input type="checkbox" className="w-5 h-5 accent-blue-600" onChange={(e) => setFiltros({...filtros, soloExtranjeros: e.target.checked})} />
              <label className="text-[10px] text-gray-300">🌐 Solo Propiedades que Aceptan Extranjeros</label>
            </div>
          </div>
        </aside>
              ...
              <div key={p.id} className="bg-[#111] p-4 rounded-2xl border border-white/10 hover:border-red-500 transition-all duration-300 hover:-translate-y-2 flex flex-col h-full relative">
                <div className="cursor-pointer" onClick={() => setPropiedadSeleccionada(p)}>
                  <div className="relative h-40 overflow-hidden rounded-xl mb-3 bg-gray-900">
                    ...
                    {p.esExtranjero && (
                      <div className="absolute top-2 right-2 z-20 px-2 py-0.5 bg-blue-600 text-white text-[9px] font-black uppercase rounded">🌐 Extranjeros</div>
                    )}
                    {p.imagenes?.map((img, i) => (
                      <img key={i} src={img} className={`absolute w-full h-full object-contain transition-opacity duration-1000 ${i === (indicesFotos[p.id] || 0) ? 'opacity-100' : 'opacity-0'}`} alt="propiedad" />
                    ))}
                  </div>
                ...
                <button 
                  ...
                  className={`mt-4 w-full py-2 rounded-lg text-[11px] font-bold transition-colors ${favoritos.includes(p.id) ? "bg-red-600 text-white" : "bg-white/5 text-white hover:bg-red-600"}`}>
                  ❤️ FAVORITO
                </button>
                <a href={`https://wa.me/18090000000?text=Hola, interesado en: ${p.titulo}`} target="_blank" className="mt-2 w-full py-2 bg-green-600 hover:bg-green-700 rounded-lg text-[11px] font-bold text-white text-center transition-colors">💬 WHATSAPP</a>
              </div>
            ))}
          </div>
        </main>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
          ...
          <div className="col-span-1">
            ...
            <p className="text-gray-400 text-sm leading-relaxed">
              Tu aliado confiable en la búsqueda y adquisición de propiedades en República Dominicana.
            </p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-gray-400 font-medium">
              ...
              <li><Link href="/nosotros" className="hover:text-red-500 transition">Acerca de Nosotros</Link></li>
              <li><Link href="/Alquiler" className="hover:text-red-500 transition">Alquiler</Link></li>
              <li><Link href="/Comprar" className="hover:text-red-500 transition">Comprar</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-gray-400 font-medium">
              <li><Link href="/calculadora" className="hover:text-red-500 transition">Calculadora Hipotecaria</Link></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-gray-400 text-sm">
              <li className="flex items-start gap-2"><span>📍</span> Ubicación: República Dominicana, Santo Domingo Este, La ureña, C. El Sol</li>
              <li className="flex items-center gap-2"><span>📞</span> <a href="tel:+18498572321" className="hover:text-red-500 transition">Teléfono: +1 (849) 857-2321</a></li>
              <li className="flex items-center gap-2"><span>📧</span> <a href="mailto:Danieldecena63@gmail.com" className="hover:text-red-500 transition underline">Correo: Danieldecena63@gmail.com</a></li>
            </ul>
          </div>
        </div>
...
const Selector = ({ label, opciones, onChange, value }) => (
  <div>
    ...
    <select value={value} className="w-full bg-[#1a1a1a] p-3 rounded-lg text-sm border border-white/10 text-white" onChange={(e) => onChange?.(e.target.value)}>
