...
export default function PublicarPropiedad() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 3. Traducimos "tipo" al campo "operacion" que esperan las páginas públicas
      const operacion = datos.tipo === "venta" ? "Venta" : "Alquiler";

      // 4. Guardar datos en Firestore
      await addDoc(collection(db, "propiedades"), {
        ...datos,
        operacion,               // campo que faltaba y causaba el bug
        creadorUid: usuario.uid, // para poder identificar al dueño más adelante
        estado: "Disponible",    // para que no la excluya el filtro de "Archivada"
        fotoUrl,
        createdAt: new Date(),
      });

      alert("¡Publicada correctamente!");
      router.push(datos.tipo === "venta" ? "/Comprar" : "/Alquiler");
    } catch (error) {
      console.error(error);
